...
var Sheet = function () {
	function Sheet() {
		...
		this.cells = Array(101).fill().map(function () {
			return Array(101);
		});
	...
	_createClass(Sheet, [{
		...
		value: function refresh() {
			...
			this.el = (0, _jquery2.default)(document.createElement('table'));
			// Initialize the grid
			for (var i = 0; i <= 100; i++) {
				...
				for (var j = 0; j <= 100; j++) {
					...
					if (!this.cells[i][j]) {
						var cell = new _Cell2.default(i, j, cell);
		...
		value: function parse(str) {
			...
			if (operatorIndex === -1) {
				...
				var operand1 = parseFloat(str.substr(0, operatorIndex)) || this.parseCellData(str.substr(0, operatorIndex));
				var operand2 = this.parse(str.substr(operatorIndex + 1));
		...
		key: 'parseCellData',
		value: function parseCellData(str) {
			if (!Array.isArray(str)) {
				var strArr = Array.from(str);
			}
			// Find the first digit
			var firstDigitIndex = strArr.findIndex(this.isDigit);

			var rowStr = str.substr(firstDigitIndex);
			var colStr = str.substr(0, firstDigitIndex);
			var row = parseFloat(rowStr);
			if (isNaN(row) || row > 100 || row < 1) return NaN;
			var col = this.parseColNo(colStr);
			if (isNaN(col)) {
				return NaN;
			}
			// Cell found
		...
		value: function isBinaryOperator(char) {
		...
		key: 'parseColNo',
		value: function parseColNo(colStr) {
			var result = 0;
		...
		key: 'getRowNo',
		value: function getRowNo(cellName) {
			var firstDigitIndex = Array.from(cellName).findIndex(this.isDigit);

			var rowStr = cellName.substr(firstDigitIndex);
			var row = parseFloat(rowStr);
			if (isNaN(row) || row > 100 || row < 1) return NaN;else return row;
		}
	}, {
		key: 'getColNo',
		value: function getColNo(cellName) {
			var firstDigitIndex = Array.from(cellName).findIndex(this.isDigit);
			var colStr = cellName.substr(0, firstDigitIndex);
			var col = this.parseColNo(colStr);
			return col;
		}
	}, {
...
var _Sheet = __webpack_require__(1);
...
(0, _jquery2.default)(document).ready(function () {
	var sheet = new _Sheet2.default();
	(0, _jquery2.default)('.root').append(sheet.el);
	// Refresh the spread sheet
	(0, _jquery2.default)('#refresh').click(function () {
		sheet.refresh();
		(0, _jquery2.default)('.root').append(sheet.el);
	});
});
...
var Cell = function () {
	function Cell(row, col, cell) {
		...
		// DOM element (td)
		this.el = (0, _jquery2.default)(document.createElement('td'));
		this.row = row;
		this.col = col;
		this.data = 0;
		this.formula = null;
		...
		// If not header, add input field
				...
				this.input = (0, _jquery2.default)(document.createElement('input'));
  ...
  */
		...
		key: 'getHeaderText',
		value: function getHeaderText(colNo) {
			var letters = " ABCDEFGHIJKLMNOPQRSTUVWXYZ";
			if (colNo % 26 == 0) {
